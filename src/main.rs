...
impl Recorder {
    fn start() -> Result<Self> {
        ...
        stream.play()?;
        Ok(Self { samples, stream: Some(stream) })
    }
...
fn samples_to_wav(samples: &[i16]) -> Result<Vec<u8>> {
    let spec = WavSpec {
...
fn is_too_short(samples: &[i16]) -> bool {
    samples.len() < SAMPLE_RATE as usize / 2
}

fn process(samples: Vec<i16>, cfg: Arc<Config>) {
    let result = (|| -> Result<()> {
        if is_too_short(&samples) {
...
mod tests {
    ...
    fn wav_magic_bytes_are_correct() {
        let wav = samples_to_wav(&[0i16; 100]).unwrap();
    ...
    fn wav_samples_round_trip() {
        ...
        let wav = samples_to_wav(&original).unwrap();
    ...
    fn recording_too_short_is_detected() {
        assert!(is_too_short(&vec![0i16; SAMPLE_RATE as usize / 2 - 1]));
    }

    #[test]
    fn recording_at_threshold_is_accepted() {
        assert!(!is_too_short(&vec![0i16; SAMPLE_RATE as usize / 2]));
    }
}
            ...
            if ev.id == toggle.id() && ev.state == global_hotkey::HotKeyState::Pressed {
                match recorder.take() {
                    ...
                    Some(r) => {
                        ...
                        let samples = r.stop();
                        let cfg = cfg.clone();
                        thread::spawn(move || process(samples, cfg));
                    }
                }
            }
        }
